...
class EventsService:
    ...
    async def get_events(
        ...
        for event in result.value:
            ...
            if filter_categories:
                ...
                filter_categories_lower = [cat.lower().strip() for cat in filter_categories]
    ...
    def _parse_event(self, event) -> Dict[str, Any]:
        ...
        status = format_enum(str(event.response_status.response)) if event.response_status and event.response_status.response else ""
        ...
        is_recurring = event.type and str(event.type).lower() in ["occurrence", "seriesmaster"]
        recurrence_pattern = ""
        if event.recurrence and event.recurrence.pattern and event.recurrence.pattern.type:
            recurrence_pattern = str(event.recurrence.pattern.type).lower().capitalize()
        
        online_meeting_url = ""
        if event.is_online_meeting and event.online_meeting and event.online_meeting.join_url:
        ...
        return {
            ...
            "organizer": event.organizer.email_address.address if event.organizer and event.organizer.email_address else "",
    ...
    @staticmethod
    def process_description(description: str, mode: str = "full", max_length: Optional[int] = None) -> str:
