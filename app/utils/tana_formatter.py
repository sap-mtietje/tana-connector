...
class TanaFormatter:
    ...
    @staticmethod
    def format_node(
        title: str, children: List[str] = None, fields: Dict[str, Any] = None
    ) -> str:
    ...
    def format_events(
        ...
        filter_fields_lower = (
            [f.lower() for f in filter_fields] if filter_fields else []
        )
        ...
        for event in events:
            ...
            TanaFormatter._add_field(
                lines, "Identifier", event.get("id"), filter_fields_lower, show_empty
            )

            # Date field (combined start/end range)
            if TanaFormatter._should_display(
                "Date", event.get("date"), filter_fields_lower, show_empty
            ):
            ...
            if TanaFormatter._should_display(
                "Start", event.get("start"), filter_fields_lower, show_empty
            ):
                lines.append(
                    f"  - Start:: {TanaFormatter._format_single_datetime(event['start'])}"
                )

            # End field (individual end time)
            if TanaFormatter._should_display(
                "End", event.get("end"), filter_fields_lower, show_empty
            ):
                lines.append(
                    f"  - End:: {TanaFormatter._format_single_datetime(event['end'])}"
                )

            TanaFormatter._add_tagged_field(
                lines,
                "Location",
                event.get("location"),
                field_tags.get("location"),
                filter_fields_lower,
                show_empty,
            )
            TanaFormatter._add_field(
                lines, "Status", event.get("status"), filter_fields_lower, show_empty
            )
            TanaFormatter._add_field(
                lines,
                "Availability",
            ...
            TanaFormatter._add_tagged_field(
                ...
                "Organizer",
                event.get("organizer"),
                field_tags.get("organizer"),
                filter_fields_lower,
                show_empty,
            )

            TanaFormatter._add_list_field(
                lines,
                "Attendees",
                event.get("attendees", []),
                field_tags.get("attendees"),
                filter_fields_lower,
                show_empty,
            )
            TanaFormatter._add_list_field(
            ...
            if TanaFormatter._should_display(
                "Reminder", event.get("is_reminder_on"), filter_fields_lower, show_empty
            ):
            ...
            TanaFormatter._add_bool_field(
                lines,
                "Has Attachments",
                event.get("has_attachments"),
                filter_fields_lower,
                show_empty,
            )

            if TanaFormatter._should_display(
                "Description", event.get("description"), filter_fields_lower, show_empty
            ):
    ...
    @staticmethod
    def _should_display(
        field: str, value, filter_fields: List[str], show_empty: bool
    ) -> bool:
    ...
    @staticmethod
    def _add_field(
        lines: List[str], name: str, value, filter_fields: List[str], show_empty: bool
    ):
    ...
    def _add_list_field(
        ...
        if (
            TanaFormatter._should_display(name, items, filter_fields, show_empty)
            and items
        ):
            lines.append(f"  - {name}:: ")
            for item in items:
    ...
    def _format_timing_eventlink(start: str, end: str) -> str:
        ...
        try:
            start_dt = datetime.fromisoformat(
                start.replace("Z", "").replace(".0000000", "")
            )
            end_dt = datetime.fromisoformat(
                end.replace("Z", "").replace(".0000000", "")
            )
